...
export default function BookOffice() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        // Set state for office data
        setOffice(officeData);

        // Generate a unique code
        const generatedUniqueCode = Math.floor(100 + Math.random() * 900); // Random 3-digit code
        const grandTotal = officeData.price - generatedUniqueCode;

        // Update unique code and total amount
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} action="booking-finished.html" className="relative flex justify-center max-w-[1130px] mx-auto gap-[30px] mb-20 z-20">
        <div className="flex flex-col shrink-0 w-[500px] h-fit rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {formErrors.find((error) => error.path.includes('started_at')) && <p className="text-red-500">Started At is required</p>}
            </div>
          </div>
          <hr className="border-[#F6F5FD]" />
        ...
        <div className="flex flex-col shrink-0 w-[400px] h-fit rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
          ...
          <hr className="border-[#F6F5FD]" />
          <button disabled={isLoading} type="submit" className="flex items-center justify-center w-full rounded-full p-[16px_26px] gap-3 bg-[#0D903A] font-bold text-[#F7F7FD]">
